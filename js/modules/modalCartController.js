  ...
  if (cartControl.cartData.length) {
    ...
    const cardsData = await Promise.all(
      cartControl.cartData.map(
        async (item) =>
          await getData(
            `https://complex-beautiful-kiwi.glitch.me/api/products/${item.id}`
          )
      )
    );
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch(
      "https://complex-beautiful-kiwi.glitch.me/api/orders/",
      {
        method: "POST",
        headers: {
